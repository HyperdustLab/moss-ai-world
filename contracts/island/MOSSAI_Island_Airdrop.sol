...
contract MOSSAI_Island_Airdrop is OwnableUpgradeable {
    ...
    function addIslandAirdrop(string memory name, uint256[] memory uint256Array, uint256 startTime, uint256 endTime, string memory airdropConfig, bytes1 status, uint256 intervalTime) public {
        require(IHyperAGI_Roles_Cfg(_rolesCfgAddress).hasAdminRole(msg.sender), "not admin role");

        if (uint256Array[3] != 0) {
            (uint256 _id, , , , , , , , , ) = MOSSAI_Island(_islandAddress).getIsland(uint256Array[3]);
        ...
        storageAddress.setUint(storageAddress.genKey("islandId", id), uint256Array[3]);
    ...
    function updateIslandAirdrop(uint256 id, string memory name, uint256 minRandomAmount, uint256 maxRandomAmount, uint256 startTime, uint256 endTime, string memory airdropConfig, uint256 intervalTime, uint256 totalAmount, uint256 islandId) public {
        require(IHyperAGI_Roles_Cfg(_rolesCfgAddress).hasAdminRole(msg.sender), "not admin role");

        MOSSAI_Storage storageAddress = MOSSAI_Storage(_storageAddress);

        if (islandId != 0) {
            (uint256 _id, , , , , , , , , ) = MOSSAI_Island(_islandAddress).getIsland(islandId);
        ...
        storageAddress.setUint(storageAddress.genKey("startTime", id), startTime);
        storageAddress.setUint(storageAddress.genKey("endTime", id), endTime);
        storageAddress.setUint(storageAddress.genKey("islandId", id), islandId);
    ...
    function withdraw(uint256 id, uint256 amount) public {
        ...
        (, , , , , , , , uint32 seed, ) = MOSSAI_Island(_MOSSAIIslandAddress).getIsland(islandId);
